...
function calculateDueDate(submitDate, turnAroundTime) {                    
  // the function receives a submitdate in timestamp format and a turnaround time in a string
  let hoursGivenInTheForm = parseInt(turnAroundTime, 10);                  
  // parsing the number in the string
  let dueDateWithoutOvertimeHandling = 0;                             
  
  let daysToWorkingHours = Math.floor(hoursGivenInTheForm / 8) * 24;        
  // 8 hours count as a day
  let remainingHours = hoursGivenInTheForm % 8; 
  let allTheHoursTogether = daysToWorkingHours + remainingHours; 
  let timeTillDeadLine = allTheHoursTogether * 1000 * 60 * 60;              
  // getting the time till deadline in timestamp format
  
  dueDateWithoutOvertimeHandling = submitDate + timeTillDeadLine;           
  //the time of finishing the task, timestamp format - can fall to overtime and on weekends
   
  let getTheHours = new Date(dueDateWithoutOvertimeHandling).getUTCHours();                         
